...
#define DELAY_TIME_MS 50 // 延时时间，单位为10毫秒
static int blink_task(const char *arg)
{
    ...
    while (1) {
        osDelay(DELAY_TIME_MS);    // 延时500ms
        // uapi_gpio_toggle(GPIO_10); // 蜂鸣器电平反转
        uapi_gpio_set_val(GPIO_13, GPIO_LEVEL_HIGH);  // 高电平亮灯
        osDelay(DELAY_TIME_MS);    // 延时500ms
        uapi_gpio_set_val(GPIO_13, GPIO_LEVEL_LOW);   // 低电平灭灯
