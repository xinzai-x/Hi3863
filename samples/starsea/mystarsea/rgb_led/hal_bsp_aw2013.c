...
// 向aw2013的寄存器写数据
uint32_t aw2013_WiteByte(uint8_t regAddr, uint8_t byte)
{
    uint32_t result = 0;
...
{
    uint32_t result = 0;
    result = aw2013_WiteByte(PWM0_REG_ADDR, PWM_Data); // R
    if (result != ERRCODE_SUCC) {
...
{
    uint32_t result = 0;
    result = aw2013_WiteByte(PWM1_REG_ADDR, PWM_Data); // G
    if (result != ERRCODE_SUCC) {
...
uint32_t AW2013_Control_RGB(uint8_t R_Value, uint8_t G_Value, uint8_t B_Value)
{
    ...
    result = aw2013_WiteByte(PWM0_REG_ADDR, R_Value); //  R
    if (result != ERRCODE_SUCC) {
        printf("I2C aw2013 Green PWM0_REG_ADDR status = 0x%x!!!\r\n", result);
        return result;
    }

    result = aw2013_WiteByte(PWM1_REG_ADDR, G_Value); // G
    if (result != ERRCODE_SUCC) {
        printf("I2C aw2013 Red PWM1_REG_ADDR status = 0x%x!!!\r\n", result);
        return result;
    }
