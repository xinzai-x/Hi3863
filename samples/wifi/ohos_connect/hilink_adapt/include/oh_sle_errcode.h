...
#ifndef OH_SLE_ERR_CODE_TYPE_H
#define OH_SLE_ERR_CODE_TYPE_H
 ...
 */
typedef enum {
    OH_ERRCODE_SSAP_INVALID_PDU = OH_ERRCODE_SLE_SSAP_BASE + 0x01,                /*!< @if Eng invalid PDU
                                                                                       @else   服务端接收的 PDU 无效
                                                                                       @endif */
    OH_ERRCODE_SSAP_PDU_NOT_SUPPORT = OH_ERRCODE_SLE_SSAP_BASE + 0x02,            /*!< @if Eng PDU not support
                                                                                       @else   服务端不支持处理接收的
                                                                                               PDU
                                                                                       @endif */
    OH_ERRCODE_SSAP_UNKNOW = OH_ERRCODE_SLE_SSAP_BASE + 0x03,                     /*!< @if Eng PDU unknow
                                                                                       @else   服务端执行请求时发生
                                                                                               未知错误
                                                                                       @endif */
    OH_ERRCODE_SSAP_INVALID_HANDLE = OH_ERRCODE_SLE_SSAP_BASE + 0x04,             /*!< @if Eng invalid handle
                                                                                       @else   请求中的句柄无效
                                                                                       @endif */
    OH_ERRCODE_SSAP_INSUFFICIENT_RESOURCES = OH_ERRCODE_SLE_SSAP_BASE + 0x05,     /*!< @if Eng insufficient resources
                                                                                       @else   服务端没有足够资源完成
                                                                                               请求
                                                                                       @endif */
    OH_ERRCODE_SSAP_PROHIBIT_READING = OH_ERRCODE_SLE_SSAP_BASE + 0x06,           /*!< @if Eng prohibit reading
                                                                                       @else   服务端禁止客户端读取值
                                                                                       @endif */
    OH_ERRCODE_SSAP_PROHIBIT_WRITE = OH_ERRCODE_SLE_SSAP_BASE + 0x07,             /*!< @if Eng prohibit writing
                                                                                       @else   服务端禁止客户端写入值
                                                                                       @endif */
    OH_ERRCODE_SSAP_CLIENT_NOT_AUTHENTICATED = OH_ERRCODE_SLE_SSAP_BASE + 0x08,   /*!< @if Eng client no authentication
                                                                                       @else   客户端未经过认证
                                                                                       @endif */
    OH_ERRCODE_SSAP_CLIENT_NOT_AUTHORIZATION = OH_ERRCODE_SLE_SSAP_BASE + 0x09,   /*!< @if Eng client no authorizaion
                                                                                       @else   客户端未被授权
                                                                                       @endif */
    OH_ERRCODE_SSAP_BEARER_NOT_ENCRYPTED = OH_ERRCODE_SLE_SSAP_BASE + 0x0A,       /*!< @if Eng bearer not encrypted
                                                                                       @else   传输 PDU 的承载未加密
                                                                                       @endif */
    OH_ERRCODE_SSAP_ENTRIES_NOT_FOUND = OH_ERRCODE_SLE_SSAP_BASE + 0x0B,          /*!< @if Eng entries not found
                                                                                       @else   服务端未找到对应条目
                                                                                       @endif */
    OH_ERRCODE_SSAP_DATA_NOT_FOUND = OH_ERRCODE_SLE_SSAP_BASE + 0x0C,             /*!< @if Eng data not found
                                                                                       @else   服务端未找到对应类型数据
                                                                                       @endif */
    OH_ERRCODE_SSAP_INCORRECT_DATA_TYPE = OH_ERRCODE_SLE_SSAP_BASE + 0x0D,        /*!< @if Eng data type incorrected
                                                                                       @else   客户端发送写入数据类型
                                                                                               不符的错误
                                                                                       @endif */
    OH_ERRCODE_SSAP_INCORRECT_DATA_VALUE = OH_ERRCODE_SLE_SSAP_BASE + 0x0E,       /*!< @if Eng data value incorrected
                                                                                       @else   客户端发送写入值不符
                                                                                               的错误
                                                                                       @endif */
    OH_ERRCODE_SSAP_VALUE_OUT_OF_RANGE = OH_ERRCODE_SLE_SSAP_BASE + 0x0F,         /*!< @if Eng data value incorrected
                                                                                       @else   客户端写入的值超出范围
                                                                                       @endif */
    OH_ERRCODE_SSAP_UPPERLAYER_APPLICATION_ERROR_MIN = OH_ERRCODE_SLE_SSAP_BASE + 0xAF, /*!< @if Eng reserve
                                                                                       @else   预留给上层协议定义应用
                                                                                               错误
                                                                                       @endif */
    OH_ERRCODE_SSAP_UPPERLAYER_APPLICATION_ERROR_MAX = OH_ERRCODE_SLE_SSAP_BASE + 0xFF, /*!< @if Eng reserve
                                                                                       @else   预留给上层协议定义应用
                                                                                               错误
                                                                                       @endif */
} ErrCode_SsapType;
